...
const TaxList = ({
  ...
  const generatePDF = useCallback(() => {
    const doc = new jsPDF();
    const tableColumn = ["#", "Tax Code", "Tax Type", "Tax percentage", "Tax component", "Active"];
  ...
  return (
    <div className="bg-blue-400 p-8 min-h-screen">
      <header className="text-center bg-white py-2 border border-blue-300 rounded-full mb-5">
        <h2 className="text-lg font-bold text-blue-500">Tax List</h2>
      </header>
              ...
              {filteredCustomers.length > 0 ? (
                filteredCustomers.map((customer, index) => (
                  <tr key={customer.customerAccountNo}>
                    ...
                    >
                      {customer.customerAccountNo}
                    </td> <td className="border px-4 py-2 cursor-pointer text-black whitespace-normal break-words">{customer.type}</td>
                    <td className="border px-4 py-2 cursor-pointer text-black whitespace-normal break-words">{customer.name}</td>
              
                    <td className="border px-4 py-2 cursor-pointer text-black whitespace-normal break-words">{customer.type2}</td>
