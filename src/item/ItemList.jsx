...
const ItemList = ({ items, toggleView, handleDeleteItems, onAddItem, setCurrentView, onViewItem }) => {
  ...
  return (
    ...
    <header className="text-center bg-white py-2 border border-blue-300 rounded-full mb-5">
        <h2 className="text-lg font-bold  text-blue-500">Item List</h2>
      </header>
            ...
            {filteredItems.length > 0 ? (
              filteredItems.map((item, index) => (
                <tr key={item.itemNo}>
                  ...
                  <td className="border px-4 py-2 whitespace-normal break-words" style={{ maxWidth: "15ch" }}>{item.type}</td>
                  <td className="border px-4 py-2 whitespace-normal break-words" style={{ maxWidth: "15ch" }}>{item.description}</td>
                  <td className="border px-4 py-2 w-44 whitespace-normal break-words" style={{ maxWidth: "15ch" }}>{item.unit}</td>
                  <td className="border px-4 py-2 whitespace-normal break-words" style={{ maxWidth: "15ch" }}>{item.price}</td>
